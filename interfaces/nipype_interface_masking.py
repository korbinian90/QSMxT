...
def threshold_masking(in_files, threshold=None, fill_strength=None):
    ...
    if isinstance(threshold, tuple):
        threshold_small, threshold_filled = threshold
    else:
        threshold_small = threshold
        threshold_filled = threshold

    # remove noisy background voxels (applied to masks only)
    small_masks = [binary_opening(np.array(data > threshold_small, dtype=int)) for data in all_float_data]

    # hole-filling (applied to filled_masks only)
    filled_masks = [fill_holes_smoothing(np.array(data > threshold_filled, dtype=int)) for data in all_float_data]
...
class MaskingInputSpec(BaseInterfaceInputSpec):
    ...
    threshold = traits.Union(traits.Tuple(), traits.Float())
...
class MaskingInterface(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        if isinstance(threshold, tuple):
            self._results['threshold'] = threshold[0]
        else:
